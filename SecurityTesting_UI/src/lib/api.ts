...
export const API = {
  ...
  async uploadConfig(file: File): Promise<ConfigUploadResponse> {
    ...
    const response = await api.post<ConfigUploadResponse>('/upload-config', formData, {
  ...
  async startAnalysis(
    ...
    const response = await api.post<AgentAnalysisResponse>(`/api/agent/analyze?${params.toString()}`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return response.data;
  },
  ...
  async pollResult(sessionId: string): Promise<AgentAnalysisResponse> {
    const response = await api.get<AgentAnalysisResponse>(`/api/agent/result/${sessionId}`);
    return response.data;
  },

  async getAnalyzerResult(sessionId: string): Promise<AgentAnalysisResponse> {
    const response = await api.get<AgentAnalysisResponse>(`/api/agent/analyzer-result/${sessionId}`);
    return response.data;
  },
