...
class TestStep(BaseModel):
    step: int
    description: str
    path_params: Optional[Dict[str, str]] = Field(default_factory=dict, description="Các biến trên URL path, VD: {'walletId': '{{walletId}}'}")
    ...
    if is_vulnerable:
        ...
        for vuln_type in vulns:
            ...
            sys_prompt = """You are a professional security tester. Based on the endpoint information and the OWASP KB, generate a detailed security testing plan.
                            RULES:
                        1. DYNAMIC VARIABLES FORMAT: The endpoint consumes these base variables: {consumes_variables}. 
                        - You MUST ALWAYS use DOUBLE BRACES for placeholders (e.g., `{{{{variableName}}}}`).
                        - If the test requires multiple users (e.g., BOLA/IDOR testing), you MUST strictly append `_A` and `_B` to the base variable names. 
                        - EXACT ALLOWED FORMAT (they are examples - but format like that: val name + A/B/C): `{{{{walletId_A}}}}`, `{{{{walletId_B}}}}`, `{{{{accessToken_A}}}}`, `{{{{accessToken_B}}}}`.
                        - DO NOT invent new naming conventions like `UserA`, `_userA`, or camelCase variations.
        ...
        sys_prompt = """You are an automated API tester. This endpoint is NOT vulnerable, but it MUST be executed to generate prerequisite data ({produces_variables}) for subsequent tests.
                    ...
                    2. DYNAMIC VARIABLES: The endpoint needs dynamic IDs from previous steps: {consumes_variables}. 
                    - You MUST use DOUBLE BRACES for placeholders. 
                    - Example: If the variable is "username", you MUST write `{{{{username}}}}`. DO NOT use single braces like `{{username}}`.
